    ...
    <section className="relative min-h-screen bg-[linear-gradient(to_bottom_right,var(--purple-light),var(--cream))] pt-20 pb-10 px-6 sm:px-12 flex flex-col-reverse sm:flex-row items-center justify-between">
      ...
      <div className="hex-container">
        {hexes.map((hex, idx) => (
          ...
          >
            {hex.type === "image" && (
              <img src={hex.src} alt={hex.alt} className="w-full h-full object-cover"/>
            )}
          </div>
        ))}
      </div>
      ...
      {/* Right Content */}
     <div className="text-center sm:text-right mt-8 sm:mt-0 max-w-md flex flex-col justify-end h-full">
        ...
        <div className="signature-text mb-4">— Savitha & Chethana</div>
        <div className="flex flex-col sm:flex-row gap-4 mt-6 justify-center sm:justify-end">
          <button
